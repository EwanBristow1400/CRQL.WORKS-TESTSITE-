...
(function() {
    ...
    // Generate unique ID
    function generateId() {
        return 'liquid-glass-' + Math.random().toString(30).substr(2, 9);
    }
    ...
    // Create the liquid glass effect (circular + follows mouse)
    function createLiquidGlass() {
        ...
        const shader = new Shader({
            ...
            fragment: (uv, mouse) => {
                ...
                // circular signed-distance field
                const d = Math.sqrt(x * x + y * y) - 0.33;

                // edge falloff
                const t = smoothStep(0.8, 0, d);

                return texture(x * t + 0.5, y * t + 0.5);
            }
        });
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            // keep inside screen
            const constrained = shader.constrainPosition(x, y);

            shader.container.style.left = constrained.x + 'px';
            shader.container.style.top = constrained.y + 'px';
